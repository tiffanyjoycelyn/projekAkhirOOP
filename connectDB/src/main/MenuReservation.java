...
import connectDB.SearchDB;
import connectDB.ShowDB;
import connectDB.CheckDB;
...
public class MenuReservation {
	...
	static String menuStatus = null;
	
	static boolean canHaveNewReservation() {
		if(menuStatus == null){          // DISINIII
			return true;
		}else {
			return false;
		}
	...
	static void showMenuAvailable(Connection conn, Integer branchID) {
		ShowDB.showMenutoAdd(conn, "select * from menus", branchID);
	}
	...
	static void menuReserve(Scanner ns, Connection conn) {
		...
		Integer branchID = CheckDB.checkEmployeeBranch(employeeID, conn, "select * from employees");
		BRANCH_ID = branchID;
		...
		if(branchID == 0) {
			...
			String employeeName = SearchDB.searchEmployeeName(employeeID, conn, "select * from employees");
			String branchName = SearchDB.searchBranchNameFromBranchID(branchID, conn, "select * from restaurant_branchs");
			...
			for(int i = 0; i<numTables; i++) {
				...
				Connect.insertReservation(conn, employeeID, customerName, numTables, typeID, totalPerson);
			...
			if(input == 1) {
				...
				do {
					...
					do {
						...
						menuID = ns.nextInt();
						if(CheckDB.checkmenuIDontheBranch(branchID, menuID, conn, "select * from menus") == true) {
							found = true;
						}else {
					...
					is_specialMenu = SearchDB.searchIsSpecialMenuFromMENUID(menuID, conn, "select * from menus");
					buff_branch_id = SearchDB.searchMenuBranchIDFromMENUID(menuID, conn, "select * from menus");
					buff_price = SearchDB.searchMenuPriceFromMENUID(menuID, conn, "select * from menus");
					buffmenuName=SearchDB.searchMenuNameFromMENUID(menuID, conn, "select * from menus");
					
					totalPrice += buff_price;
					
					if(is_specialMenu == 1) {
						buff_narrative = SearchDB.searchNarrativeFromMENUID(menuID, conn, "select * from menus");
						buff_location = SearchDB.searchLocationFromMENUID(menuID, conn, "select * from menus");
