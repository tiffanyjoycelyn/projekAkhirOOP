...
public class Main {
	...
	static void menuUtama (Scanner ns, Connection conn,reservations r, Employee e) {
		...
		while(input != 3) {
			...
			if(input == 1) {
				if(MenuReservation.canHaveNewReservation() == true) {
					MenuReservation.menuReserve(ns, conn);
				}else if(MenuReservation.canHaveNewReservation() == false){
					System.out.println("Sorry there is uncompleted orders, please complete it");
					MenuReservation.orderMenuPage(ns, conn, "In Order", MenuReservation.BRANCH_ID, r, e);;
				}
			}
			if (input == 2) {
